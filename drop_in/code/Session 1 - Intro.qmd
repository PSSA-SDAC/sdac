...
```{r}
install.packages("tidyverse") # for tidy data/data cleaning
library(tidyverse)
```
...
max(numbers_list) # Returns the maximum value in 'numbers', which is 10.

median(numbers_list) # Returns the median value of 'numbers', which is 5.5.

mean(numbers_list) # Returns the mean (average) value of 'numbers', which is 5.5.
...
# 9. GPT's Attempt at a Scatter Plot (1)
...
# 10. GPT's Attempt at a Scatter Plot (2)
